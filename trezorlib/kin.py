...
def parse_transaction_bytes(tx_bytes):
    """Parses base64data into a map with the following keys:
        tx - a KinSignTx describing the transaction header
        operations - an array of protobuf message objects for each operation
    """
    tx = messages.KinSignTx()
...
def _parse_operation_bytes(unpacker):
    ...
    if type == OP_CREATE_ACCOUNT:
        return messages.KinCreateAccountOp(
    ...
    if type == OP_MANAGE_OFFER:
        return messages.KinManageOfferOp(
            source_account=source_account,
    ...
    if type == OP_CREATE_PASSIVE_OFFER:
        return messages.KinCreatePassiveOfferOp(
            source_account=source_account,
            selling_asset=_xdr_read_asset(unpacker),
            buying_asset=_xdr_read_asset(unpacker),
            amount=unpacker.unpack_hyper(),
    ...
    if type == OP_SET_OPTIONS:
        op = messages.KinSetOptionsOp(source_account=source_account)
    ...
    if type == OP_CHANGE_TRUST:
        return messages.KinChangeTrustOp(
            source_account=source_account,
            asset=_xdr_read_asset(unpacker),
    ...
    if type == OP_ACCOUNT_MERGE:
        return messages.KinAccountMergeOp(
            source_account=source_account,
            destination_account=_xdr_read_address(unpacker),
        )
...
def _xdr_read_asset(unpacker):
    """Reads a kin Asset from unpacker"""
    asset = messages.KinAssetType(type=unpacker.unpack_uint())
...
def _xdr_read_address(unpacker):
    """Reads a kin address and returns the string representing the address
...
@expect(messages.KinAddress, field="address")
def get_address(client, address_n, show_display=False):
    return client.call(
        messages.KinGetAddress(address_n=address_n, show_display=show_display)
    )
...
def sign_tx(
    ...
    # 3. Receive a KinTxOpRequest message
    # 4. Send operations one by one until all operations have been sent. If there are more operations to sign, the device will send a KinTxOpRequest message
    # 5. The final message received will be KinSignedTx which is returned from this method
    resp = client.call(tx)
    try:
        while isinstance(resp, messages.KinTxOpRequest):
            resp = client.call(operations.pop(0))
    except IndexError:
        # pop from empty list
        raise CallException(
            "Kin.UnexpectedEndOfOperations",
            "Reached end of operations without a signature.",
        ) from None

    if not isinstance(resp, messages.KinSignedTx):
        raise CallException(messages.FailureType.UnexpectedMessage, resp)

    if operations:
        raise CallException(
            "Kin.UnprocessedOperations",
