...
from trezorlib import debuglink, messages as proto, kin
...
@pytest.mark.kin
class TestMsgKinGetAddress(TrezorTest):
    def test_kin_get_address(self):
        self.setup_mnemonic_nopin_nopassphrase()

        address = kin.get_address(
            self.client, parse_path(kin.DEFAULT_BIP32_PATH)
        )
    ...
    def test_kin_get_address_fail(self):
        self.setup_mnemonic_nopin_nopassphrase()

        with pytest.raises(CallException) as exc:
            kin.get_address(self.client, parse_path("m/0/1"))
